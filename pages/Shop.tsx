...
const Shop: React.FC = () => {
  const foodieRef = useRef<HTMLDivElement>(null);
  const goodiesRef = useRef<HTMLDivElement>(null);

  const scrollToSection = (ref: React.RefObject<HTMLDivElement>) => {
    ref.current?.scrollIntoView({ behavior: 'smooth' });
  };

  const goodies = PRODUCTS.filter(p => p.id <= 4);
  const food = PRODUCTS.filter(p => p.id > 4);
  ...
  return (
    <div className="animate-fade-in max-w-6xl mx-auto px-4 py-16">
      <div className="text-center mb-10">
        ...
        <div className="flex flex-col items-center gap-4 mb-16">
          ...
          <div className="flex items-center gap-2 text-horizon-indigo font-bold">
            <i className="fas fa-arrow-up text-2xl text-horizon-yellow"></i>
        ...
        <div className="flex justify-center gap-4 mb-12 flex-wrap">
          <button 
            onClick={() => scrollToSection(goodiesRef)}
            className="bg-horizon-indigo text-white px-8 py-3 rounded-sm font-black uppercase hover:bg-horizon-royal transition-all shadow-lg"
          >
            <i className="fas fa-gift mr-2"></i> Goodies
          </button>
          <button 
            onClick={() => scrollToSection(foodieRef)}
            className="bg-horizon-yellow text-horizon-indigo px-8 py-3 rounded-sm font-black uppercase hover:bg-yellow-300 transition-all shadow-lg"
          >
            <i className="fas fa-apple-alt mr-2"></i> Produits alimentaires
          </button>
        </div>
      </div>

      {/* Section Goodies */}
      <div ref={goodiesRef} className="mb-24 scroll-mt-20">
        <h2 className="text-4xl font-black text-horizon-indigo mb-8 text-center uppercase tracking-tighter">
          <i className="fas fa-gift mr-3 text-horizon-yellow"></i>Goodies
        </h2>
      ...
      <div ref={foodieRef} className="mb-24 scroll-mt-20">
        <h2 className="text-4xl font-black text-horizon-indigo mb-8 text-center uppercase tracking-tighter">
          <i className="fas fa-apple-alt mr-3 text-horizon-yellow"></i>Produits alimentaires
        </h2>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {food.map((product) => (
          <div key={product.id} className="bg-white border-2 border-horizon-light rounded-sm overflow-hidden shadow-2xl group flex flex-col h-full">
            <div className="h-[300px] overflow-hidden bg-horizon-light relative">
              <div className="absolute top-4 left-4 bg-horizon-yellow text-horizon-indigo px-4 py-1 font-black text-xs uppercase z-10 shadow-lg">Produit</div>
              <img 
                src={product.image} 
                alt={product.name} 
                className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700"
